...
func main() {
	...
	fmt.Fprintf(w, "// https://github.com/AvengeMedia/dankgo/cmd/go-wayland-scanner\n")
	...
	if protocol.Name != "wayland" {
		fmt.Fprintf(w, "import \"github.com/AvengeMedia/dankgo/wayland/client\"\n")
		fmt.Fprintf(w, "import xdg_shell \"github.com/AvengeMedia/dankgo/wayland/xdg_shell\"\n")
	}
...
func fmtFile(b []byte) []byte {
	...
	if err == nil && len(out) > 0 {
		langVersion = "go" + strings.TrimPrefix(string(out), "go")
	}
...
`, ifaceName)
	...
	// Interface struct
	fmt.Fprintf(w, "// %s : %s\n", ifaceName, new(doc.Package).Synopsis(v.Description.Summary))
...
func writeEnum(w io.Writer, ifaceName string, e Enum) {
	...
	fmt.Fprintf(w, "// %s%s : %s\n", ifaceName, enumName, new(doc.Package).Synopsis(e.Description.Summary))
	...
	for _, entry := range e.Entries {
		...
		if entry.Summary != "" {
			fmt.Fprintf(w, "// %s%s%s : %s\n", ifaceName, enumName, entryName, new(doc.Package).Synopsis(entry.Summary))
		}
...
func writeEvent(w io.Writer, ifaceName string, e Event) {
	...
	// Event struct
	fmt.Fprintf(w, "// %s%sEvent : %s\n", ifaceName, eventName, new(doc.Package).Synopsis(e.Description.Summary))
	fmt.Fprint(w, comment(e.Description.Text))
	...
	for _, arg := range e.Args {
		...
		if arg.Description.Summary != "" {
			fmt.Fprintf(w, "// %s %s\n", argName, new(doc.Package).Synopsis(arg.Description.Summary))
		}
			...
			case "object", "new_id":
				if arg.Interface != "" {
					...
					if protocol.Name == "wayland" {
						...
						fmt.Fprintf(w, "%sID := client.Uint32(data[l :l+4])\n", argNameLower)
					}
